var ERROR = "ERROR";
var db = window.openDatabase("ToanHybrid", "1.0", "TOAN", 20000);
...
function confirmProperty(e) {
  ...
  var propertyCity = $(
    "#page-create #frm-register #city option:selected"
  ).text();
  var propertyDistrict = $(
    "#page-create #frm-register #district option:selected"
  ).text();
  var propertyWard = $(
    "#page-create #frm-register #ward option:selected"
  ).text();
...
$(document).on("vclick", "#btn-cordova-vibrate", cordovaVibration);
...
function search(e) {
  ...
  db.transaction(function (tx) {
    ...
    if (propertyCity !== "Select city") {
      query += ` propertyCity LIKE "%${propertyCity}%"   AND`;
      var propertyDistrict = $(
        "#page-search #frm-search #district option:selected"
      ).text();
      console.log(propertyDistrict);

      if (propertyDistrict !== "Select District") {
        query += ` propertyDistrict LIKE "%${propertyDistrict}%"   AND`;
        var propertyWard = $(
          "#page-search #frm-search #ward option:selected"
        ).text();

        if (propertyWard !== "Select Ward") {
